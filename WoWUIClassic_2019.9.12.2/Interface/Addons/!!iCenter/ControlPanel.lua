...
if GetLocale() == "zhCN" then
	FF_NameWC	= "世界频道";
	FF_DescWC	= "点击加入/退出大脚世界频道";
	FF_WC		= "大脚世界频道";
	FF_JOIN		= "已加入大脚世界频道";
	FF_LEAVE	= "已退出大脚世界频道";
elseif GetLocale() == "zhTW" then
	FF_NameWC	= "世界頻道";
	FF_DescWC	= "点击加入/退出大腳世界頻道";
	FF_WC		= "大腳世界頻道";
	FF_JOIN		= "已加入大腳世界頻道";
	FF_LEAVE	= "已退出大腳世界頻道";
else
	FF_NameWC	= "World Channel";
	FF_DescWC	= "Click to join/leave the world channel.";
	FF_WC		= "大脚世界频道";
	...
	EarthFeature_AddButton(
		{
			...
			callback= function() ACP.SlashHandler() end;
		}
	);
end
...
if enabled > 0 then
	if GetLocale() == "zhCN" then
		FF_NameCombuctor	= "背包设定";
		FF_DescCombuctor	= "设定背包整合的各种参数";
	elseif GetLocale() == "zhTW" then
		FF_NameCombuctor	= "背包設定";
...
if enabled > 0 then
	if GetLocale() == "zhCN" then
		...
		FF_DescClassicCodex	= "全能型任务数据库";
	elseif GetLocale() == "zhTW" then
		FF_NameClassicCodex	= "任務助手";
		FF_DescClassicCodex	= "全能型任務資料庫";
	else
...
if enabled > 0 then
	if GetLocale() == "zhCN" then
		FF_NamePhanxChat	= "聊天增强";
		FF_DescPhanxChat	= "增强聊天体验（复制网址、姓名上色、允许方向键等）";
	elseif GetLocale() == "zhTW" then
		FF_NamePhanxChat	= "聊天增強";
...
local enabled = GetAddOnEnableState(playerName, "Grid");
if enabled > 0 then
	if GetLocale() == "zhCN" then
		FF_NameGrid	= "团队面板";
