...
local UnitIsUnit, UnitExists, UnitName, UnitInRaid, UnitInParty, GetNumGroupMembers = UnitIsUnit, UnitExists, UnitName, UnitInRaid, UnitInParty, GetNumGroupMembers
    ...
    if db["TargetsTooltipUnit"] ~= 2 then
        ...
        if UnitExists(target) then
            ...
            if isself then
                ...
                if ( isPlayerOrPet and UnitCanAttack(target, "player") ) or not UnitIsTapDenied(unit) or
...
--  used to be getnumraidmembers,  may be able to fold into above now

    num = GetNumGroupMembers() or 0
