...
function module.TooltipFormat(unit, name, realm, isPlayer, isPlayerOrPet, isDead)
    ...
    local i, line, lineText, levelLine, afterLevelLine, guildLine
    ...
    if db["PvPRankText"] ~= 2 then
        _, rankNumber = GetPVPLifetimeStats()
    end
    -- begin modify by Isler
    NameColors = ColourPlayer(unit)
    name = format("|cff"..NameColors.."%s|r", name)
    -- end modify
    if isPlayer then
    ...
    elseif ( isPlayerOrPet and UnitCanAttack(unit, "player") ) or not UnitIsTapDenied(unit) or
    ...
    if afterLevelLine and afterLevelLine > 3 then
	    ...
	    if t and GameTooltipTextLeft2:IsShown() then
               GameTooltipTextLeft2:SetText("<" .. t .. ">")
               GameTooltipTextLeft2:SetTextColor(GameTooltipTextLeft1:GetTextColor())      end
        end
