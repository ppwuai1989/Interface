...
function CSC_PaperDollFrame_SetDamage(statFrame, unit, category)
	...
	if ( offhandSpeed and category == "近战") then
...
function CSC_PaperDollFrame_SetHitChance(statFrame, unit)
	...
	CSC_PaperDollFrame_SetLabelAndText(statFrame, "命中", hitChanceText, true, hitChance);
...
function CSC_PaperDollFrame_SetHealing(statFrame, unit)
	...
	CSC_PaperDollFrame_SetLabelAndText(statFrame, "治疗", healingText, false, healing);
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE.."额外治疗"..FONT_COLOR_CODE_CLOSE;
	statFrame:Show();
end
