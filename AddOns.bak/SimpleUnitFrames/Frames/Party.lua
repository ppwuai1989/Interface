...
if CI then
	...
	CI:RegisterEvent("GROUP_ROSTER_UPDATE", function()
		for i = 1, 4 do
			CI:UpdateClassIcon("party"..i)
		end
	end)
	CI:RegisterEvent("UNIT_CONNECTION", function(_, unit)
		CI:UpdateClassIcon(unit)
	end)
...
if TO then
	TO:RegisterEvent("GROUP_ROSTER_UPDATE", function()
		for i = 1, 4 do
			TO:RefreshFontStrings("party"..i)
		end
	end)
	TO:RegisterEvent("UNIT_CONNECTION", function(_, unit)
		TO:RefreshFontStrings(unit)
	end)
	for i = 1, 4 do
		local partyFrame = "PartyMemberFrame"..i
